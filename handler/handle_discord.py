...
class DiscordHandler():
    ...
    def embed_fields_from_trade(self, generated_trade:dict, rolimon_data:dict, projected_data) -> dict:
        ...
        total_profit, algorithm_profit, rap_profit, value_profit = self.trade_profit(generated_trade)
    ...
    def setup_embed(self, title:str, color, user_id, embed_fields:dict, footer:str, description=""):#, author_icon):
        """
        ...
        try:
            thumbnail = requests.get(f"https://thumbnails.roblox.com/v1/users/avatar-headshot?userIds={user_id}&size=48x48&format=Png&isCircular=false").json()['data'][0]['imageUrl']
        except:
        ...
        for field_name, (field_value, inline) in embed_fields.items():
            embed.add_embed_field(name=field_name, value=field_value, inline=inline)
    ...
    def send_webhook(self, embed:DiscordEmbed, webhook_url: str):
        ...
        try:
            ...
            webhook = DiscordWebhook(url=webhook_url, username="Doggo Tradebot")
